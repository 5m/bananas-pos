BINARY_NAME := bananas-printer
APP_NAME := Bananas Printer
CMD_DIR := ./cmd/bananas-printer
DIST_DIR := dist
LOCAL_BIN_DIR := .bin
...
WINDOWS_SYSO := cmd/bananas-printer/bananas-printer_windows_amd64.syso
...
clean:
	rm -rf "$(DIST_DIR)" "cmd/bananas-printer/$(APP_NAME).app" "$(WINDOWS_SYSO)"
...
package-macos-app: $(FYNE_BIN) $(MACOS_APP_ICON_ICNS)
	mkdir -p "$(DIST_DIR)"
	rm -rf "cmd/bananas-printer/$(APP_NAME).app" "$(DIST_DIR)/$(APP_NAME).app"
	cd cmd/bananas-printer && \
	...
	cp "$(MACOS_APP_ICON_ICNS)" "cmd/bananas-printer/$(APP_NAME).app/Contents/Resources/icon.icns"
	mv "cmd/bananas-printer/$(APP_NAME).app" "$(DIST_DIR)/"
